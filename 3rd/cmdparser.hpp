...
#include <vector>
    ...
    class Parser {
      private:
        ...
        parse(const std::vector<std::string> &elements,
              const unsigned long long &, int numberBase = 0) {
            if (elements.size() != 1)
                throw std::bad_cast();
      ...
      public:
        ...
        Parser(int argc, const char **argv,
            ...
            : _appname(argv[0]), _general_help_text(std::move(
...
#pragma warning(pop)
        ...
        template <typename T>
        void set_default(bool is_required,
        ...
        void set_required(const std::string &name,
                          ...
                          const std::string &description = "",
                          bool dominant = false) {
            auto command = new CmdArgument<T>{name, alternative, description,
                                              true, dominant};
            _commands.push_back(command);
        }
        ...
        void set_optional(const std::string &name,
                          const std::string &alternative, T defaultValue,
                          const std::string &description = "",
                          bool dominant = false) {
            auto command = new CmdArgument<T>{name, alternative, description,
                                              false, dominant};
        ...
        void set_callback(const std::string &name,
                          ...
                          const std::string &description = "",
                          bool dominant = false) {
            auto command = new CmdFunction<T>{name, alternative, description,
                                              false, dominant};
                ...
                if (command->name == name) {
                    ...
                    if (cmd == nullptr) {
                        throw std::runtime_error(
                            "Invalid usage of the parameter " + name +
                            " detected.");
                    }
